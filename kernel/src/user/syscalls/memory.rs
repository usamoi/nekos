...
impl_errno!(MEMORY_CREATE_INVALID_LAYOUT, 0x69c9cf35u32);
...
impl Syscalls<{ Syscall::MEMORY_CREATE }> for Syscall {
    ...
    async fn syscall(
        env: &Environment,
        (size, align, ..): syscall_domain!(),
    ) -> EffSys<Self::Codomain> {
        use MemoryCreateError::*;
        let layout = MapLayout::new(size, align).ok_or(Errno::MEMORY_CREATE_INVALID_LAYOUT)?;
